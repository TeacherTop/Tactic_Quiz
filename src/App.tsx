...
export default function App() {
  ...
  return <div className={`arena ${phase === 'home' ? '' : 'game-shell'}`}>
    ...
    <main className="stage">
      ...
      {phase === 'expansion' && (timeline.step === 'announcing' || timeline.step === 'pre-timer') ? <TimelineOverlay progress={timeline.progress}><RoundAnnouncement phase={phase} match={match} /></TimelineOverlay> : null}
      ...
      {timeline.resultVisible && completedRoundResult?.kind === 'quiz' ? <RoundResultOverlay result={completedRoundResult} humanCorrect={match?.expansionAnswers.you === completedRoundResult.question.correctIndex} progress={timeline.progress} /> : null}
...
function TimelineOverlay({ children, progress }: { children: ReactNode; progress: number }) {
  return <motion.div className="timeline-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: ANIMATION_TIMINGS.minTransition / 1000 }}>
    {children}
    <PauseProgress progress={progress} />
  </motion.div>
}
